# -*- coding: utf-8 -*-
"""
Created on Mon Oct 17 15:27:56 2022
...
def load_data():
    data_file = open('CBF_shadow_data.p', 'rb')
    data = pickle.load(data_file)
    data_file.close()

    data_file = open('CBF_shadow_labels.p', 'rb')
...
if __name__ == "__main__":
    ...
    tensors = model(x_train)
    with tf.GradientTape() as tape:
        tape.watch(tensors)
        loss = loss_object(tf.Variable(y_train), tensors)
    ...
    locals()['shadow_model_dataframe_CBF'] = pd.DataFrame(data=d)

    pickle.dump(locals()['shadow_model_dataframe_CBF'], open('shadow_model_dataframe_CBF.p', 'wb'))
