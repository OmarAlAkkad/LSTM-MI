...
def load_data():
    data_file = open(f'target_model_dataframe_enddays.p', 'rb')
    actual = pickle.load(data_file)
    data_file.close()

    data_file = open(f'shadow_model_dataframe_enddays.p', 'rb')
...
if __name__ == "__main__":
    ...
    x_train, y_train, x_test, y_test, indices_train, indices_test, months_train, months_test = load_data()
    ...
    d.to_csv(f'attack_models_class.csv')
    ...
    plt.figure()
    plt.title('Shadow model success rate')
    plt.bar(number_of_months,train_months_percentage)
    plt.figure()
    plt.title('Actual model success rate')
