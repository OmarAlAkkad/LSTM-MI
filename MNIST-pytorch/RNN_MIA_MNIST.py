# -*- coding: utf-8 -*-
"""
Created on Sun Apr  9 22:07:58 2023
...
import random
import numpy as np
#@title (Run) Part 3: Prepare mnist10 dataset for target and shadow model
import pickle
...
def create_mnist_dataset_torch(name, load_data = False, batch_size=128, target_train_size = 15000, target_test_size= 15000, shadow_train_size = 15000, shadow_test_size= 15000):
  ...
  if not load_data:
      ...
      mnist_dataset = torch.utils.data.ConcatDataset([mnist_trainset, mnist_testset])


      #target_train_size = int(0.25 * len(mnist_dataset)) # 15000
      remain_size = len(mnist_dataset) - target_train_size
      target_train_dataset, remain_dataset = torch.utils.data.random_split(mnist_dataset, [target_train_size, remain_size])

      #target_test_size = int(0.25 * len(mnist_dataset)) # 15000
      ...
      #shadow_train_size = int(0.25 * len(mnist_dataset)) # 15000
...
if __name__ == "__main__":
    ...
    for data,method_name,save_model_folder,name in models:
        target_trainloader, target_testloader, shadow_trainloader, shadow_testloader = create_mnist_dataset_torch(data, load_data = False, batch_size=64, target_train_size = 17500, target_test_size= 17500, shadow_train_size = 17500, shadow_test_size= 17500)
