# -*- coding: utf-8 -*-
"""
Created on Thu Sep 22 09:30:16 2022
...
def preprocess_dataset():
    ...
    for i in range(len(daily_data)):
        daily_data['Date'][i] = f"{daily_data.index[i].day}-{daily_data.index[i].month}-{daily_data.index[i].year}"
    daily_data = label_days(daily_data)
    daily_data = get_month(daily_data)
...
def label_days(dataset):
    i = 0
    j = 0
    dataset['Label'] = np.ones(len(dataset['Global_active_power']), dtype=int)
    while j < len(dataset['Global_active_power']):
        if i >= 24*7:
            i = 0
        if 24*5 <= i < 24*7:
            dataset['Label'][j] = int(0)
        j += 1
        i += 1
...
def get_month(data):
    data['month_of_date'] = np.ones(len(data))
    for i in range(len(data)):
        data['month_of_date'][i] = data.index[i].month
    return data

def split_labels(data, label):
    split = pd.DataFrame()
    for i in range(len(data)):
        if data['Label'][i] == int(label):
            split = split.append(data.iloc[i], ignore_index=True)
    split.set_index(['Date'], inplace=True)
    return split

def to_days(data, seq_size):
    x = []
    columns = data.columns
    for i in range(0,len(data)- 2*seq_size, seq_size):
        x.append(np.array(data.iloc[i:(i + seq_size), :]))
    np.random.shuffle(x)
...
if __name__ == '__main__':
    ...
    past_hours = 24
    dataset_days = to_days(dataset, past_hours)

    split = 0.5
    weekdays = split_labels(dataset_days,1)
    weekends = split_labels(dataset_days,0)

    weekdays_split = (round(len(weekdays)/24*split))*24
    weekends_split = (round(len(weekends)/24*split))*24

    actual_model = [weekdays.iloc[0:int(weekdays_split),:], weekends.iloc[0:int(weekends_split),:]]
    actual_model_data = pd.concat(actual_model,  ignore_index=False)

    shadow_model = [weekdays.iloc[int(weekdays_split):len(weekdays),:], weekends.iloc[int(weekends_split):len(weekends),:]]
    shadow_model_data = pd.concat(shadow_model,  ignore_index=False)
